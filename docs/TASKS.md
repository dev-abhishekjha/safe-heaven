...
| `[x]` | **E7.7** Lightbox click-through | **Verified 2026-09-22** in a real browser on the deployed site, after three earlier attempts failed for environment reasons. Clicked a thumbnail, confirmed the dialog, stepped forward and back with the arrow keys, confirmed the caption and counter, closed with Escape |
...
| `[x]` | **E14.6** Keyboard nav + visible focus | **Driven in a real browser against the live site, 2026-09-22.** Lightbox: opens, traps focus, arrow keys step (1→2→3 and back), the live region announces each photo, Escape closes, background content is `aria-hidden`. One defect found and fixed — focus was NOT restored on close, in either dialog. Radix restores to `Dialog.Trigger`, and neither the enquiry modal nor the gallery uses one, so `document.activeElement` ended up on `<body>` and a keyboard user was dropped at the top of the document. `useReturnFocus` fixes both. `PropertyGallery`'s docblock had claimed the opposite for weeks |
