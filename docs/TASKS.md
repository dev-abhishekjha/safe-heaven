...
| `[x]` | **E16.12** Vercel cannot reach Postgres | **Resolved, cause not provable.** `/diag` run from a production function in iad1 on 2026-09-21: DNS returns two A records and no AAAA, sockets open to 5432 and 6543 in ~250ms, a real login succeeds on both, every env var matches the laptop by sha256 fingerprint, `pg.Pool` completes a checkout, a transaction and four concurrent queries, and Payload initialises, reads, counts and opens a transaction. Three things changed between the failures and this: `DATABASE_URI` moved to the transaction pooler, `max` went 1 → 10, and Supabase's own dashboard was showing "we are investigating a technical issue" during the failures. Any of them could have been it and the evidence does not separate them, so this is recorded as fixed, not as understood. If it returns, `/diag` (E16.13) is the first thing to restore |
| `[~]` | **E16.13** Delete the `/diag` route | Temporary. Remove the route, `DIAG_TOKEN` from Vercel, the entry in `.env.example`, and `scripts/check-db.mjs` and `scripts/check-ports.mjs` with it, once E16.12 stays closed. Those two scripts are why `npm run lint` covers `src` only — they use `console.log`, which the config forbids, and they are not worth exempting for the days they have left |
| `[!]` | **E16.20** Migration for the widened icon enums | Adding `hospital` and `shopping` to the page globals and `pin`, `opportunity` and `chat` to amenities changes five Postgres enums. Run `npm run migrate:create` then `npm run migrate` BEFORE `npm run seed` — locally `push` is on (NODE_ENV is not `production` under `payload run`), so seeding first would auto-push the change and leave a `batch: -1` row in `payload_migrations`, which is exactly what hung the Vercel build for fifteen minutes |
