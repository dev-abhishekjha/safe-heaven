...
| `[~]` | **E16.12** "timeout exceeded when trying to connect" | **Reopened, and no longer a Vercel problem.** `npm run migrate` produced the identical error from a laptop that had just connected to the same database in 446ms, and a seed run succeeded against it seconds later. The cause of THAT failure is `max: 1` for migration runs: `payload migrate` initialises Payload, which does its own database work, and the migration runner then asks for a second connection that queues behind it and times out. Fixed by giving migrations a pool of 5. Whether the earlier Vercel failures were the same thing is untested — runtime used `max: 1` until c65fad0, which is consistent, but a Supabase incident was also live at the time. `/diag` stays until it has not recurred |
...
| `[~]` | **E16.20** Reconcile `payload_migrations` | `npm run migrate` timed out, then `npm run seed` ran and its push applied the icon enum change anyway. So the live schema is correct but `20260921_190119_icon_options` was never recorded as applied, and push will have left a `batch: -1` marker. The migration cannot simply be run — it is `ALTER TYPE ... ADD VALUE` with no `IF NOT EXISTS`, so it would fail on labels that already exist. It has to be baselined: drop the `-1` row and insert the migration's name |
